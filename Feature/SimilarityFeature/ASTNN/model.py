...
class BatchProgramPretrain(nn.Module):
    ...
    def __init__(
        ...
        super(BatchProgramPretrain, self).__init__()
        ...
        # linear
        self.hiddenlayer = nn.Linear(self.hidden_dim * 2, self.out_dim)
        # hidden
...
class BatchProgramClassifier(nn.Module):
    ...
    def __init__(
        ...
        # class "BatchTreeEncoder"
        self.emblayer = BatchProgramPretrain(
            self.embedding_dim,
            self.hidden_dim,
            self.vocab_size,
            self.encode_dim,
        ...
        # linear
        self.hidden2input = nn.Linear(self.out_dim, 32)
        self.hidden2label = nn.Linear(32, self.label_size)
        # hidden
        self.dropout = nn.Dropout(0.2)

    def forward(self, x, x2):

        emb1 = self.emblayer(x)
        emb2 = self.emblayer(x2)
        emb1 = torch.unsqueeze(emb1, 1)
        emb2 = torch.unsqueeze(emb2, 1)
        # y = F.cosine_similarity(emb1,emb2,dim=2)
        abs_dist = torch.abs(torch.add(emb1, -emb2))
        out1 = self.hidden2input(abs_dist)
        output = self.hidden2label(out1)
